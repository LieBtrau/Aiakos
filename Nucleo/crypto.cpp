...
//NFC-SEC-1: ECMA-386: Key Agreement
...
bool Crypto::calcMasterKeySSE(const byte* pRemotePublicKey, const byte* pRemoteNonce, const byte* pRemoteNFCID3, bool bIsInitiator)
{
    ...
    if(!uECC_shared_secret(pRemotePublicKey, _pLocalPrivateKey, SharedSecret))
    {
        return false;
    }
    ...
    memcpy(S+BLOCK_SIZE+(bIsInitiator?0:NFCID_SIZE),_NFCID3i,NFCID_SIZE);
...
bool Crypto::testMasterKeySse(){
    ...
    memcpy(_pLocalPrivateKey, private1, uECC_BYTES);
    memcpy(_localNonce, nonce1, BLOCK_SIZE/2);
    memcpy(_NFCID3i, NFCID3_1, NFCID_SIZE);
    ...
    memcpy(_pLocalPrivateKey, private2, uECC_BYTES);
    memcpy(_localNonce, nonce2, BLOCK_SIZE/2);
    memcpy(_NFCID3i, NFCID3_2, NFCID_SIZE);
